...
- **Cloudflare Pages（全栈 Serverless，推荐）**：后端已改写为 Pages Functions，数据存 KV、上传存 R2，
  **后台管理 / AI 识别 / 访问统计在线上全部可用**。完整步骤见 [CLOUDFLARE.md](./CLOUDFLARE.md)。
  - ⚠️ 部署前请先按 [CLOUDFLARE.md](./CLOUDFLARE.md) 建好 KV 命名空间与 R2 桶，并把 KV id 填进 `wrangler.toml`——绑定由该文件管理，后台界面不提供添加（会提示 "managed through wrangler.toml"）。改动后需重新部署才生效。
  - ⚠️ 部署后到 `/admin.html` **设置后台管理员密码**（详见 [CLOUDFLARE.md](./CLOUDFLARE.md)「设置后台管理员密码」），否则后台为只读开放、密码保护不生效。
