...
绑定同一个 `NAV_KV` 后，导航数据、配置、注册账户、访客申请分别放在四个键里：

| KV 键 | 内容 | 谁写 |
| --- | --- | --- |
| `sites` | 全部导航数据（分类 + 链接） | 后台保存 / 申请审核通过后自动收录 |
| `config` | AI 配置 + 管理员账号密码 + `SESSION_SECRET` | 后台「保存设置」/ 首次初始化 |
| `accounts` | 注册账户（含 `salt` / `passHash`，**无明文密码**） | 访客注册、管理员审核与改角色 |
| `applications` | 访客提交的「添加网站」申请 | 访客提交、管理员审核 |

> 排查账户相关问题时，直接看 `accounts` 键即可：某账户 `status` 是 `pending` 就是他还没被审核、
> 是 `disabled` 就是被禁用了，此时他手里的 token 已经失效。清空 `accounts` 键等于删掉所有注册账户，
> **不会影响超级管理员登录**（超级管理员来自 `config`，是最后一道保险）。
