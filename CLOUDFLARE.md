...
### 防浏览器缓存：静态资源已带版本号（改完一般不用手动强刷）
- `index.html` / `admin.html` 里的 `css/style.css`、`js/app.js`、`js/admin.js` 已加 `?v=20260829`
  查询参数。浏览器把「带不同查询串的 URL」视为不同文件，**发版后用户打开站点会自动拉取新文件**，
  通常无需手动 `Ctrl+F5` 强刷（彻底解决此前「改了代码像没生效」的缓存痛点）。
- ⚠️ **维护约定（重要）**：版本号是「手动」的——它只解决「浏览器缓存旧文件」，**不解决「线上跑的是旧提交」**。
  每次发版改了前端代码（`app.js` / `admin.js` / `style.css`）后，**必须把 `?v=` 的日期往前拨一位**
  （如 `20260829` → `20260830`，或换成当天日期），否则浏览器仍用已缓存的旧文件，看起来「改了没生效」。
  推荐做法：发版当天统一把两个 html 里的 `?v=` 改成当天日期。
- ⚠️ 两级「生效」要分清：
  1. **上线生效**：改了代码必须 `git push` → Cloudflare 自动重新部署（或手动 Redeploy），否则线上跑的还是旧文件；
  2. **客户端生效**：版本号变更后，用户浏览器才会放弃缓存去拉新文件。
  两者都满足，改动能真正被看到。只看版本号不 push，或只 push 不改版本号，都可能出现「看起来没生效」。
